...
two pointer approach better hobe. jehetu 2nd string ta 1st string e thkte hobe, so
jdi ? pawa jay taile nicher string ta iterate korbo or same letter hoile iterate krbo.
ekhane iterate j diye korar por sure eta last e 2nd string er last prjonto chole jabe. 
jodi jay taile yes; possible otherwise it's not possible.
...
for(int i=0;i<str.size();i++){
    if(str[i]=='?'){
       if(j<str1.size()) {str[i] = str1[j];
        j++;}

        else str[i] = 'a';
    }
    else if (str[i]==str1[j]) j++;
