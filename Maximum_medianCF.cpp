...
bool ok(vl v, ll k, ll mid, ll med ){ // 1 2 3 4 4 4 4, 7     1 2 3 5 7 8 9, 7

    for(ll i = med; i < v.size(); i++){
       ...
       // if(k <= 0) break;
        ...
        ll score = mid - v[i];
        if(score < 0 or score > k) return false;

        if(k <= 0 and i + 1 < v.size() and v[i + 1] < v[i] + mid ) return false;
        else {
            k -= score;
        }
