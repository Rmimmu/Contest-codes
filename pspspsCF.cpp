...
he entire p must be a permutation, if s1=s, we can set s1=., and if sn=p, 
we can set sn=.. after that, the answer is YES if and only if all non-dot characters in s
 are all p or s.
...
void solve(){
    ...
    int countp=0,counts=0,countd=0;
 ...
 (countp+countd==n or counts + countd==n) ? YES:NO;
