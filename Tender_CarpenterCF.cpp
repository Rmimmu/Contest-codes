/*
Name of the problem:A. Tender Carpenter

If we find two adjacent pair which abs diff is less then both of them them there will be answer 
as the rest can be paired by the only one number.
...
for(int i=1;i<n;i++){ 
    cin>>arr[i];
if(abs(arr[i]-arr[i-1])<arr[i]&&abs(arr[i]-arr[i-1])<arr[i-1]) ok = 1;
