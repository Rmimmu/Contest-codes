...
Solution description (if needed):
    ...
    we can form it like a zigzag format. so for that split them into two separate 
    strsings and count zeros. if the zeros are same then yes or else no.
...
void solve(){
    ...
    for(int i =0;i<n;i++){
        if(i % 2 == 0) str.push_back(a[i]);
        else str.push_back(b[i]);
    }

    for(int i=0; i < n;i++) {
        if(i % 2 == 0) str1.push_back(b[i]);
        else str1.push_back(a[i]);
    }
    ...
    (count1 >= (n+1) / 2 and count2 >= n / 2) ? YES : NO;
}
